...
* TODO Messaging [0/1]
- [ ] Find a way to avoid sending an error message several times for discrepancies in the same varible.
...
* TODO Database for strong references and settings [3/5]
...
- [ ] Implement the __del__(self) function so that the database connection is closed when the python destructor is called on the object.
